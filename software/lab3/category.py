...
class C(GrammarCategory):
    def __init__(self, *args, **features):
        ...
        self._features[self.semanticTypeName] = semanticType
        self._str = self._getStr()
        self._hash = hash(self._str)

        """
        Live dangerously: we're theoretically immutable,
        but sometimes people violate our abstraction and
    ...
    def _strParts(self):
        ...
        if "/" in keys:
            keys.remove("/")
            slash = "/" + str(self._features["/"])
        mytype = ""
        keys.remove(self.semanticTypeName)
        if self.semanticType():
            mytype = self.semanticType()
        if len(keys) > 0:
            features = [str(self._features[key]) for key in keys]
        else:
            features = ""
        if (mytype == "" and features == ""):
            mytype = "-"
        return (mytype, features, slash)

    def _getStr(self):
        return "%s%s%s" % self._strParts()
    ...
    def addFeature(self, name, value):
        """
        non-destructive add returns a new C.
        """
        newdict = dict(self._features)
        newdict[name] = value
        return C(self.semanticType(), **newdict)
    ...
    def getFeature(self, key):
        print self._features[key]
        return self._features[key]
    """


class StarCategory(C):
    """
    The special semantic category *, which matches anything.
    """
    def __init__(self):
        C.__init__(self, '*')
...
class ParserSubstitute(logic.LogicParser):
    """
...
class ApplicationExpressionSubst(logic.ApplicationExpression,
                                 SubstituteBindingsI):
    """
