...
class Tree(list):
    ...
    def productions(self):
        ...
        if not isinstance(self.node, str):
            raise TypeError, ('Productions can only be generated from trees having node labels that are strings.  Label %s is of type: %s.' % (self.node, type(self.node)))
    ...
    def _thawed_class(self): return Tree
    def freeze(self, leaf_freezer=None):
        ...
        return newcopy
    def thaw(self, leaf_thawer=None):
        thawed_class = self._thawed_class()
        if leaf_thawer is None:
            newcopy = thawed_class.convert(self)
        else:
            newcopy = self.copy(deep=True)
            for pos in newcopy.treepositions('leaves'):
                newcopy[pos] = leaf_thawer(newcopy[pos])
            newcopy = thawed_class.convert(newcopy)
	# mutable trees need not be hashable
        return newcopy
...
class ParentedTree(AbstractParentedTree):
    ...
    def _thawed_class(self): return ParentedTree
...
class MultiParentedTree(AbstractParentedTree):
    ...
    def _thawed_class(self): return MultiParentedTree
...
class ProbabilisticTree(Tree, ProbabilisticMixIn):
    ...
    def _thawed_class(self): return ProbabilisticTree
    def __repr__(self):
