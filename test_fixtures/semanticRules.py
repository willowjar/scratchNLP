...
# Data Reporter
sem.add_rule("DataReporter -> 'the' OrderAdverb 'item' 'in' Lis LIST_NAME", lambda t, order_adverb, i, in, l, list_name: getItem(mapWordToNum(order_adverb), list_name))
...
sem.add_rule("EVENT -> 'when' KEY_NAME 'is' 'pressed'", lambda w, name, is, pressed: whenKeyPress(name))
sem.add_rule("EVENT -> 'when' 'this' 'sprite' 'is' 'clicked'", lambda w, t, is, cli: whenClicked())
...
sme.add_rule("Ele -> 'element'", identity)
sme.add_rule("Ele -> 'Det element'", lambda d, e: e)
...
sem.add_rule("BP -> BP 'and' BP", lambda b1,and, bp: logicAnd(b1, b2))
sem.add_rule("BP -> BP 'or' BP", lambda b1, or, b2: logicOr(b1, b2))
...
sem.add_rule("ConditionalCommand -> 'if' BP 'then' AL Thatsit otherwise AL Thatsit", lambda i, bp, then, al1, thatsit, ow, al2, thatsit: ifElseCommand(bp,al1,al2))

sem.add_rule("ConditionalCommand -> 'if' BP 'then' AL Thatsit else AL Thatsit", lambda i, bp, then, al1, thatsit, ow, al2, thatsit: ifElseCommand(bp,al1,al2))
...
sem.add_rule("LoopCommandP -> 'the' 'following' Duration AL Thatsit", lambda t, f, duration, action_list, t: repeat_action_list(action_list, duration))
sem.add_rule("LoopCommandP -> 'the' 'following' 'steps' Duration AL Thatsit", lambda t, f, s, duration, action_list, t: repeat_action_list(action_list, duration))
